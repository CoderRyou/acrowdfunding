...
<div class="container theme-showcase" role="main">
    ...
    <form id="authcodeForm" role="form" style="margin-top:20px;">
        <div class="form-group">
            <label for="authcode">验证码</label>
            <input type="text" name="authcode" class="form-control" id="authcode" placeholder="请输入你邮箱中接收到的验证码">
        </div>
...
<script>
    ...
    $("#finishBtn").click(function () {
        $.ajax({
            url:"${APP_PATH}/member/doCheckCode.do",
            type:"POST",
            data:$("#authcodeForm").serialize(),
            beforeSubmit:function () {
                loadingIndex = layer.msg("确认验证码中",{icon:6});
                return true;//必须返回true，否则，请求终止
            },
            success:function (result) {
                layer.close(loadingIndex);
                if(result.success){
                    layer.msg("实名认证申请提交成功，请等待审核！",{time:1000,icon:6});
            ...
            error:function () {
                layer.msg("实名认证申请提交出错",{time:1000,icon:5});
            }
        });
    });
</script>
