...
<div class="container theme-showcase" role="main">
    ...
    <form id="uploadCertForm" role="form" style="margin-top:20px;" enctype="multipart/form-data">
        <c:forEach items="${sessionScope.certs}" var="cert" varStatus="status">
            <div class="form-group">
                <label for="">${cert.name}</label>
                <input type="hidden" name="certimgs[${status.index}].certid" value="${cert.id}">
                <input type="file" class="form-control" name="certimgs[${status.index}].file">
                <br>
    ...
    $("#nextBtn").click(function () {
        ...
        var options = {
            ...
            success:function (result) {
                ...
                if(result.success){
                    layer.msg("资质文件保存成功",{time:1000,icon:6});
                    window.location.href = "${APP_PATH}/member/apply.htm";
                }else{
                    layer.msg("资质文件保存失败",{time:1000,icon:5,shift:6});
                }
            },
            error:function () {
                layer.msg("资质文件保存出错",{time:1000,icon:5});
            }
        };
